...
class ConceptBertModel(BertPreTrainedModel):
    """
    ...
    def forward(
            ...
            concept_pooling='mean'
    ):
        assert concept_position is not None
        # try:
        #     for spans in concept_position:
        #         start = -1
        #         for span in spans:
        ...
        if encoder_embeds is None:
            ...
            '''check valid spans'''
            valid_concept_position = []
            for spans, mask in zip(concept_position, attention_mask):
                valid_spans = []
                valid_length = torch.sum(mask)
                for span in spans:
                    if span[0] >= valid_length or span[1] >= valid_length:
                        continue
                    valid_spans.append(span)
                valid_concept_position.append(valid_spans)
            concept_position = valid_concept_position

            '''pool concept embeddings'''
            final_concept_position = []
            merged_embeddings = []
            # import transformers
            # tokenizer = transformers.AutoTokenizer.from_pretrained('bert-base-uncased')
            assert len(concept_position) == len(embedding_output)
            for idx, (spans, embeds) in enumerate(zip(concept_position, embedding_output)):
                temp_position = []
                # print(embeds.shape)
                reduce_length = 0
                for span in spans:
                    if span[0] == -1 or span[1] == -1:
                        ...
                        span = [span[0] - reduce_length, span[1] - reduce_length]
                        if concept_pooling == 'mean':
                            concept_embed = torch.mean(embeds[span[0]:span[1]], dim=0)
                        else:
                            raise KeyError('wrong concept pooling strategy')
                        reduce_length += span[1] - span[0] - 1
                        embeds = torch.cat([embeds[:span[0]], concept_embed.unsqueeze(0), embeds[span[1]:]], dim=0)
                        # print(embeds.shape)
                        temp_position.append(span[0])
                merged_embeddings.append(embeds)
                final_concept_position.append(temp_position)

            '''adjust the attention mask and pad the embeddings accordingly'''
            null_embed = torch.zeros(embedding_output.shape[-1]).to(device)
            seq_length = input_ids.shape[1]
            for i in range(batch_size):
                origin_att_len = torch.sum(attention_mask[i])
                reduce_length = 0
                for j in range(len(concept_position[i])):
                    if concept_position[i][j][0] == -1 or concept_position[i][j][1] == -1:
                        continue
                    elif concept_position[i][j][1] == concept_position[i][j][0] + 1:
                        continue
                    else:
                        reduce_length += concept_position[i][j][1] - concept_position[i][j][0] - 1
                assert reduce_length == seq_length - len(merged_embeddings[i])
                extended_attention_mask[i, :, :, origin_att_len - reduce_length:] = -10000.
                merged_embeddings[i] = torch.cat(
                    [merged_embeddings[i], null_embed.expand(reduce_length, -1)], dim=0)
                # try:
                #     assert len(merged_embeddings[i]) == 256
                # except AssertionError:
                #     print(reduce_length)
                #     print(concept_position[i])
                #     print(input_ids[i])
                #     print(torch.sum(attention_mask[i]))
                #     exit('debug')
            try:
                embedding_output = torch.stack(merged_embeddings, dim=0)
            except RuntimeError:
                for idx, _ in enumerate(merged_embeddings):
                    print(concept_position[idx])
                    print(input_ids[idx])
                    print(torch.sum(attention_mask[idx]))
                exit('debug')
        else:
        ...
        if not return_dict:
            return (sequence_output, pooled_output) + encoder_outputs[1:], final_concept_position
...
class ConceptBertForMaskedLM(BertPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids", r"predictions.decoder.bias"]
    ...
    def __init__(self, config):
        ...
        self.bert = ConceptBertModel(config, add_pooling_layer=False)
        self.cls = BertOnlyMLMHead(config)
