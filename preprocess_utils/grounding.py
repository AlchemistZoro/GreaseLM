...
# 在构建知识图谱的时候使用，并非在数据集grounding的时候使用
...
# 采用spacy的match方式获得question和answer的concept。
...
def prune(data, cpnet_vocab_path):
    ...
    for item in tqdm(data):
        qc = item["qc"]
        prune_qc = []
        # 做一些去重,最后单词为er和e并且前面的单词再question concept里面的就跳过。
        for c in qc:
...
if __name__ == "__main__":
    ...
    ans_matcher.add("occ",[[{'TEXT': token.text.lower()} for token in ans_words]])
